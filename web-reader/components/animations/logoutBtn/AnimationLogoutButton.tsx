...
export const AnimationLogoutButton = ({
  ...
  useEffect(() => {
    ...
    const onMouseEnter = () => {
      if ((button as any).state === "default") updateButtonState(button, "hover");
    };
    const onMouseLeave = () => {
      if ((button as any).state === "hover") updateButtonState(button, "default");
    };
    const onClick = () => {
      if (
        (button as any).state === "default" ||
  ...
  return (
    <div className={`${styles.body} ${className}`} {...props}>
      ...
      >
        <svg className={`${styles.doorway} ${styles.svg}`} viewBox="0 0 100 100">
          <path
