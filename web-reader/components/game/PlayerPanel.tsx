...
export const PlayerPanel = observer(() => {
  ...
  return <div className={`fixed z-10 left-10 top-20 h-screen flex flex-col justify-start player-control ${store.showPlayerControl && "showing"}`}>
    {store.isLoggedIn ? <>
      <div className={`hidden md:flex flex-col`}>
        <div className="flex flex-col justify-center items-center  rounded-md p-0 backdrop-blur-sm bg-slate-100/10 rounded-t-full">
          ...
          <div className="flex w-full flex-col bg-slate-500 rounded-md p-4">
            <div className="font-bold text-red-700 text-center">{getLevelName(store.profile.level)}</div>
      ...
      <div className="p-6 gap-4 flex-col bg-slate-600 h-fit hidden md:flex justify-center items-center rounded-md">
        ...
        <Link href={"/login"}>
          <div className="flex flex-col justify-center items-center">
            <img width="64" height="64" src="https://img.icons8.com/glyph-neue/64/fraud.png" alt="fraud" />
            {t("please_login")}
          </div>
        </Link>
        {/* </div> */}
